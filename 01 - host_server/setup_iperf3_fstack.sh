...
#
# Este fork do F-Stack tem tres bugs conhecidos relevantes para o teste com
# iperf3 (ver Tutorial.md secoes 5 e 6 para detalhes completos). Este script
# aplica os tres automaticamente (idempotente — nao duplica os patches se
# rodado de novo):
#   1. [lib/Makefile + lib/ff_stub_14_extra.c] tcp_ecn.c (a implementacao
#      real de tcp_ecn_syncache_add/respond etc.) nunca esta listado em
#      NETINET_SRCS, entao o linker resolve essas chamadas contra stubs
#      vazios/com assinatura incompativel. O chamador (tcp_syncache.c) le
#      lixo do registrador de retorno esperando um int, corrompendo
#      sc->sc_flags com bits imprevisiveis — as vezes incluindo SCF_NOOPT,
#      que suprime TODAS as opcoes TCP do SYN-ACK (MSS cai para 536 bytes,
#      RTO entra em backoff exponencial). ESTA E A CAUSA RAIZ do throughput
#      catastroficamente baixo (~100 Kbps) visto antes desta correcao —
#      depois dela, o mesmo link chega a ~930 Mbps (linha de Gigabit).
#   2. [adapter/syscall] Crash "buffer overflow detected" no iperf3 — a
#      numeracao de fd do F-Stack (>=128) estoura o fd_set de tamanho fixo
#      usado internamente pelo iperf3 via select(); corrigido compilando
#      com FF_PRELOAD_SUPPORT_SELECT=1 (flag documentada no Makefile do
#      adaptador, mas desligada por padrao).
#   3. [adapter/syscall/ff_hook_syscall.c] "unable to set TCP_CONGESTION" —
#      optname 13 do Linux nao tem traducao para o equivalente FreeBSD
#      neste adaptador; corrigido com um patch pontual que intercepta essa
...
# Corrige o bug raiz do throughput baixo/SYN-ACK sem opcoes TCP: tcp_ecn.c
# (a implementacao real de tcp_ecn_syncache_add/respond etc.) nunca esta
# listado em lib/Makefile's NETINET_SRCS neste fork, entao o linker resolve
# essas chamadas contra stubs vazios/com assinatura incompativel em
# ff_stub_14_extra.c (ex.: void tcp_ecn_syncache_add(sc,iptos,thflags) vs a
# int tcp_ecn_syncache_add(thflags,iptos) real declarada em tcp_ecn.h). O
# chamador (tcp_syncache.c) le lixo do registrador de retorno esperando um
# int, corrompendo sc->sc_flags com bits imprevisiveis — as vezes incluindo
# SCF_NOOPT, que suprime todas as opcoes TCP do SYN-ACK (MSS cai para o
# minimo do RFC, 536 bytes, matando o throughput). Idempotente.
