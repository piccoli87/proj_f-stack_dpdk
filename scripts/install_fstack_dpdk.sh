...
FSTACK_BRANCH="${FSTACK_BRANCH:-fix/tcp-ecn-linerate-throughput}"  # branch do F-Stack a clonar
# Por padrao, clona o fork com a correcao de line-rate ja aplicada (ver
# ../f-stack-fork/ e ../docs/Tutorial.md secao 5). Para usar o F-Stack
# oficial sem a correcao (nao recomendado, ver bug documentado), defina:
#   FSTACK_REPO_URL=https://github.com/F-Stack/f-stack.git FSTACK_BRANCH=dev
FSTACK_REPO_URL="${FSTACK_REPO_URL:-https://github.com/SEU_USUARIO/f-stack.git}"
...
clone_fstack() {
    ...
    if [[ ! -d "${FSTACK_DIR}" ]]; then
        retry 5 5 git clone --branch "${FSTACK_BRANCH}" --depth 1 \
            "${FSTACK_REPO_URL}" "${FSTACK_DIR}"
    else
        log "Diretório ${FSTACK_DIR} já existe, pulando clone."
    fi

    # Redundante se FSTACK_REPO_URL apontar para o fork (branch
    # fix/tcp-ecn-linerate-throughput ja inclui isso), mas idempotente e
    # inofensivo — mantido para quem apontar FSTACK_REPO_URL para o F-Stack
    # oficial (branch dev) sem os patches deste projeto.
